...
use oxideav_h263::encoder::{
    encode_inter_picture, encode_inter_picture_sac, encode_intra_picture, encode_intra_picture_sac,
};
use oxideav_h263::picture::{
    decode_picture_no_gob0_header, decode_picture_sac, DecodeOptions, YuvFrame,
};
...
fn sac_picture_contains_no_emulated_psc() {
    ...
    for i in 0..bytes.len().saturating_sub(2) {
        if bytes[i] == 0x00 && bytes[i + 1] == 0x00 && (bytes[i + 2] & 0b1111_1100) == 0b1000_0000 {
            hits.push(i);
        }
    }
    assert_eq!(hits, vec![0], "PSC-prefix hits at {hits:?}");
}
